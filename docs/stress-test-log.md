...
|---|---------|------|--------|---------|------------|--------|
| 18 | `docopt` (checkpoint/resume) | py2→py3 | Real end-to-end validation of the new `--checkpoint-dir` feature - no prior real-world resumability existed at all (confirmed by code inspection, not just assumed). | First run with `--checkpoint-dir` set completed normally (real LLM calls, real Docker sandboxes) and wrote a checkpoint entry per file. Second run against the same checkpoint directory completed in 1.6s total (vs. the first run's real LLM/Docker time) and produced a byte-identical report (status/reason/final_source matched exactly for all 3 files) - confirmed genuinely skipping re-processing, not just happening to reproduce the same result. | none (new feature, not a bug fix) | complete |
| 17 | `argcomplete` (completers.py diagnosis) | py2→py3 | Finally root-caused the `completers.py`/`EnvironCompleter` mismatch flagged as an open, undiagnosed lead across entries 13, 15, and 16, without ever being chased down. | Not a real behavior difference: `completers.py` is a package submodule, so importing it also imports `argcomplete/__init__.py` first - which still has the unresolved `import pipes` repair failure (a confirmed model-capability ceiling, entry 16). Python 3 crashed transitively at that import, before `EnvironCompleter` was ever called - stderr had the real traceback, but Mode C discarded stderr entirely, so the crash looked exactly like a genuine behavioral difference. `EnvironCompleter` itself has no real py2/py3 difference - confirmed by reproducing it standalone, bypassing the broken import chain. | [#36](bug-log.md) (found and fixed this run) | complete |
| 16 | `docopt` + `blinker` + `argcomplete` (model-limit investigation) | py2→py3 | Re-run of the 3 reproducible repair-loop failures identified in entry 15 (`docopt.py`'s indentation corruption, `blinker/_saferef.py`'s `__cmp__` rewrite, `argcomplete/__init__.py`'s `pipes` fix), each with `--max-repair-attempts 8` (up from the default 3), to test whether the failures are budget-limited or a genuine model-capability ceiling. | Raising the budget helped none of the 3 - confirmed two distinct, real failure sub-types, neither improved by more attempts: **execution ceiling** (`docopt.py` - the Auditor diagnosed the identical correct fix, "restore 4-space indentation," on all 8 attempts; the Refactorer produced the identical broken output every time) and **diagnosis oscillation** (`blinker/_saferef.py`, `argcomplete/__init__.py` - the Auditor cycled through multiple different, sometimes directly contradictory theories across attempts - e.g. "add `__hash__`" then "remove `__hash__`" then "add it back," or "restore the `pipes` import" then "remove it" then "stub it" then "restore it again" - never converging). Investigated a possible mechanical fallback for the execution-ceiling sub-type (deterministically re-indent after a specific `IndentationError` shape) using `docopt.py`'s real corruption as the test case - found it's not safely buildable as a text-level heuristic: the corrupted block's own internal relative indentation stayed correct, only its depth *relative to its own `def` header* was wrong, and nothing in the indentation alone distinguishes where that block ends from where the next, correctly-indented sibling method begins (both land at the same indent level). A real fix would need to reach into `apply_symbol_blocks`'s splice context (knowing which symbol, and its real original depth) rather than pattern-matching the broken text alone - and even then risks producing plausible-but-wrong code on a bad guess, a worse outcome than today's honest `NEEDS_REVIEW`. Decision: do not build it; accept these as correctly-terminal outcomes. | none (investigation only - no code changed) | complete |
| 15 | `docopt` + `html2text` + `blinker` + `argcomplete` (rule-based fixes round) | py2→py3 | Re-run against all four's still-available extractions, to scope and validate the 3 concrete rule-based fixes identified from the earlier 14-file `NEEDS_REVIEW` breakdown: docopt's closure gap, html2text's dead shim, and blinker/argcomplete's class-method scope limit. | All 3 rule classes confirmed real and fixed. `docopt/example.py`: found the closure fix was never actually re-validated after #13/#15/#21/#22 landed - re-running surfaced a *different*, deeper bug (#30: closure resolution silently losing import edges for any py2-only-syntax importer), fixed; `example.py` itself still doesn't verify, blocked by a separate, reproducible Refactorer indentation failure on `docopt.py`'s own repair (see entry 1's update). `html2text.py`: the dead `True`/`False` shim (#14) now actually gets stripped (#31) instead of just diagnosed; still doesn't verify, blocked by an unrelated Mode B stderr-comparison false-positive (flagged, not fixed - see this entry's detail). `blinker/base.py`: Mode C now characterizes class methods (#32); combined with a second real bug this surfaced and fixed (#33, default-object/generator repr noise in `_values_equal`), reaches `VERIFIED_INFERRED` (6/6). `argcomplete/my_argparse.py`: confirmed correctly `UNVERIFIED` - its one method is genuinely private, nothing for the new capability to legitimately test. | [#30](bug-log.md), [#31](bug-log.md), [#32](bug-log.md), [#33](bug-log.md) (all found and fixed this run) | complete |
| 14 | `toolz` (Mode R stress test) | py2→py3 | [pytoolz/toolz](https://github.com/pytoolz/toolz)'s real `dicttoolz.core.merge` - first real third-party stress test of Mode R/record-replay (docs/verification.md), which had previously only been validated against a small hand-written example. Recorded real calls under real Python 2, replayed against toolz's real implementation and a deliberately broken one. | Found and fixed a real, serious correctness bug in the recording wire format itself (JSON silently coercing non-string dict keys) before it could ever produce a false mismatch on real code. After the fix: 5/5 real recorded `merge()` calls correctly matched toolz's real implementation (`VERIFIED_RECORDED`-equivalent PASS); 4/5 correctly failed against a deliberately broken candidate, with the 5th (a single-dict merge, structurally unaffected by the specific bug introduced) correctly still passing - real per-case signal, not a blanket failure. | [#29](bug-log.md) (found and fixed this run) | complete |
| 13 | `argcomplete` | py2→py3 | [kislyuk/argcomplete](https://github.com/kislyuk/argcomplete) @ `f6a7bf4` — third library scouted to stress differential fuzzing/confidence counts on unseen code, last of this round. | `__init__.py`'s real 43-line test suite (`test/test.py`, singular directory) was invisible to Mode A entirely until a directory-naming gap was fixed; then hit a real `import pipes` crash (removed in Python 3.13) until a new detector was added. Landed `NEEDS_REVIEW` in the end - not from a ShiftCode gap, but a real, observed Auditor misdiagnosis (told the Refactorer to restore the exact broken `pipes` import after a correct fix), an LLM-judgment limitation, same category as `docopt`'s documented non-determinism (entry 1). `completers.py` FAIL from fuzzing looks like a real, separate, not-yet-diagnosed issue. | [#26](bug-log.md), [#27](bug-log.md) (both found and fixed this run) | complete |
| 12 | `blinker` | py2→py3 | [jek/blinker](https://github.com/jek/blinker) @ `c06a79a` — second library scouted to stress differential fuzzing/confidence counts on unseen code (`--characterization-fuzz-cases 30 --capture-repair-history`). | `_utilities.py` went from `VERIFIED_INFERRED` (Mode C fuzzing) to real `VERIFIED` (2/2, Mode A) once a real test-pairing gap was fixed. `_saferef.py` correctly routed to Mode A too, and correctly landed `NEEDS_REVIEW` on a genuine, hard `__cmp__`→rich-comparison rewrite the configured model couldn't complete within the repair budget - an honest degrade, not a bug; a new detector now at least surfaces the full scope of that fix upfront instead of the Auditor discovering it late. | [#24](bug-log.md), [#25](bug-log.md) (both found and fixed this run) | complete |
| 11 | `toolz` | py2→py3 | [pytoolz/toolz](https://github.com/pytoolz/toolz) @ `498fefa` — first library scouted specifically to stress differential fuzzing and the confidence-count fields against genuinely unseen real code (`--characterization-fuzz-cases 30 --capture-repair-history`), not a re-run. Real deep nested multi-package structure (`toolz/itertoolz/`, `toolz/dicttoolz/`, `toolz/functoolz/`, each with their own `tests/` subdir). | 5 files (`dicttoolz/core.py`, `functoolz/core.py`, `itertoolz/recipes.py`, `utils.py`, `curried.py`) went from 0 verified to real `VERIFIED` via genuine Mode A runs, after three real bugs were found and fixed (two dependency-closure gaps, one missing semantic-findings detector). `itertoolz/core.py`'s one remaining failure traced to a pre-existing resource-hungry test in toolz's own suite (`range(1_000_000_000)` on Python 2 - confirmed OOM-killed directly against the real sandbox), not a ShiftCode bug. | [#21](bug-log.md), [#22](bug-log.md), [#23](bug-log.md) (all found and fixed this run) | complete |
...
**Update (2026-07-23, entry 15):** the shim is now actually removed (bug #31), not just diagnosed - the file no longer hits this `SyntaxError` at all. Still `NEEDS_REVIEW`, now for an unrelated Mode B stderr-comparison false positive (flagged, not fixed - see entry 15).
...
looked like the same failure. Worth re-examining; flagged as a real
candidate for the model-limit investigation phase rather than assumed to be
non-determinism a second time.
