...
Table rows are a one-line pointer, not the record — jump to `#N` in
"Entry detail" below for the full write-up (date, source, outcome, bugs
found, status).

| # | Library | Pair | Outcome | Bugs found | Status |
|---|---------|------|---------|------------|--------|
| 18 | `docopt` (checkpoint/resume) | py2→py3 | Real end-to-end validation of `--checkpoint-dir` — second run resumed in 1.6s, byte-identical report | none (new feature) | complete |
| 17 | `argcomplete` (completers.py diagnosis) | py2→py3 | Root-caused a mismatch open since entry 13 — a transitive import crash, not a real behavior difference | [#36](bug-log.md#36-mode-c-discarded-stderr-entirely-a-transitive-import-crash-on-one-side-was-indistinguishable-from-a-genuine-behavioral-difference) | complete |
| 16 | `docopt` + `blinker` + `argcomplete` (model-limit investigation) | py2→py3 | Raising `max_repair_attempts` 3→8 fixed none of 3 known reproducible failures — confirmed a hard model ceiling | none (investigation only) | complete |
| 15 | `docopt` + `html2text` + `blinker` + `argcomplete` (rule-based fixes round) | py2→py3 | All 3 scoped rule-based fixes confirmed real; 2 further bugs found and fixed along the way | [#30](bug-log.md), [#31](bug-log.md), [#32](bug-log.md), [#33](bug-log.md) | complete |
| 14 | `toolz` (Mode R stress test) | py2→py3 | First real third-party Mode R test — found and fixed a serious wire-format bug before it could ever false-mismatch real code | [#29](bug-log.md) | complete |
| 13 | `argcomplete` | py2→py3 | Landed `NEEDS_REVIEW` on a real Auditor misdiagnosis, not a ShiftCode gap; `completers.py` mismatch left open (closed in entry 17) | [#26](bug-log.md), [#27](bug-log.md) | complete |
| 12 | `blinker` | py2→py3 | `_utilities.py` reached real `VERIFIED`; `_saferef.py` correctly `NEEDS_REVIEW` on a hard rewrite the model couldn't complete | [#24](bug-log.md), [#25](bug-log.md) | complete |
| 11 | `toolz` | py2→py3 | 5 files went from 0 verified to real `VERIFIED`; one remaining failure traced to a pre-existing OOM in toolz's own test | [#21](bug-log.md), [#22](bug-log.md), [#23](bug-log.md) | complete |
| 10 | full corpus regression re-run | py2→py3 | 6/8 matched exactly; 2 regressed from an interaction with an earlier fix, then fixed to a *stronger* result than before | [#20](bug-log.md) | complete |
| 9 | `requests` | py2→py3 | First real multi-file library post-sandboxing — 3 files reach `VERIFIED_INFERRED`/real Mode A; network-dependent tests correctly fail in `--network none` | [#18](bug-log.md) | complete |
| 8 | `purl` (re-validated, nested) | py2→py3 | Real multi-file dependency-aware sandboxing closed bug #13 for real — `__init__.py` reaches genuine `VERIFIED` | [#13](bug-log.md), [#15](bug-log.md), [#16](bug-log.md), [#17](bug-log.md) | complete |
| 7 | `schedule` | py2→py3 | Clean confirmation run — 9 auto-generated tests pass, `mock` dependency correctly provisioned | none (confirmation run); see #20's later update | complete |
| 6 | `html2text` | py2→py3 | Hit an obscure py2.2-era shim producing a confusing raw-traceback message instead of a clean diagnosis | [#14](bug-log.md) | complete |
| 5 | `purl` | py2→py3 | Found a real vacuous-pass variant — identical test-collection failure on both sides scored as a match | [#12](bug-log.md), [#13](bug-log.md) (deferred, later fixed — see update) | complete |
| 4 | `jsonschema` | py2→py3 | Real 209-case test suite now runs via Mode A (was 12); 206/209 pass, 3 genuine remaining failures correctly left `NEEDS_REVIEW` | [#9](bug-log.md), [#10](bug-log.md), [#11](bug-log.md) | complete |
| 3 | `inflection` | py2→py3 | Reaches real `VERIFIED` — real parametrized pytest suite, all outcomes match | [#5](bug-log.md), [#6](bug-log.md) (didn't block), [#8](bug-log.md) | complete |
| 2 | `python-slugify` | py2→py3 | Reaches real `VERIFIED` after 3 rounds of real fixes (crash isolation, missing dependency, 2 bytes/str traps) | [#3](bug-log.md), [#5](bug-log.md), [#7](bug-log.md), [#8](bug-log.md); see #20's later update | complete |
| 1 | `docopt` | py2→py3 | Reaches real `VERIFIED` — first file in the whole corpus to do so end-to-end on real historical code | [#1](bug-log.md), [#2](bug-log.md) | complete — strongest result so far |

## Entry detail

### 18. `docopt` (checkpoint/resume)

- **Date:** 2026-07-23
- **Why this run:** real end-to-end validation of the new `--checkpoint-dir` feature — no prior real-world resumability existed at all (confirmed by code inspection, not just assumed).
- **Outcome:** first run with `--checkpoint-dir` set completed normally (real LLM calls, real Docker sandboxes) and wrote a checkpoint entry per file. Second run against the same checkpoint directory completed in 1.6s total (vs. the first run's real LLM/Docker time) and produced a byte-identical report (status/reason/final_source matched exactly for all 3 files) — confirmed genuinely skipping re-processing, not just happening to reproduce the same result.
- **Bugs found:** none — new feature, not a bug fix.
- **Status:** complete.

### 17. `argcomplete` (completers.py diagnosis)

- **Date:** 2026-07-23
- **Why this run:** finally root-caused the `completers.py`/`EnvironCompleter` mismatch flagged as an open, undiagnosed lead across entries 13, 15, and 16, without ever being chased down.
- **Outcome:** not a real behavior difference. `completers.py` is a package submodule, so importing it also imports `argcomplete/__init__.py` first — which still has the unresolved `import pipes` repair failure (a confirmed model-capability ceiling, entry 16). Python 3 crashed transitively at that import, before `EnvironCompleter` was ever called — stderr had the real traceback, but Mode C discarded stderr entirely, so the crash looked exactly like a genuine behavioral difference. `EnvironCompleter` itself has no real py2/py3 difference — confirmed by reproducing it standalone, bypassing the broken import chain.
- **Bugs found:** [#36](bug-log.md) (found and fixed this run).
- **Status:** complete.

### 16. Model-limit investigation: `docopt` + `blinker` + `argcomplete`

- **Date:** 2026-07-23
- **Why this run:** re-ran the 3 reproducible repair-loop failures identified in entry 15 (`docopt.py`'s indentation corruption, `blinker/_saferef.py`'s `__cmp__` rewrite, `argcomplete/__init__.py`'s `pipes` fix), each with `--max-repair-attempts 8` (up from the default 3), to test whether the failures are budget-limited or a genuine model-capability ceiling.
- **Outcome:** raising the budget helped none of the 3 — confirmed two distinct, real failure sub-types, neither improved by more attempts:
  - **Execution ceiling** (`docopt.py`) — the Auditor diagnosed the identical correct fix, "restore 4-space indentation," on all 8 attempts; the Refactorer produced the identical broken output every time.
  - **Diagnosis oscillation** (`blinker/_saferef.py`, `argcomplete/__init__.py`) — the Auditor cycled through multiple different, sometimes directly contradictory theories across attempts (e.g. "add `__hash__`" then "remove `__hash__`" then "add it back," or "restore the `pipes` import" then "remove it" then "stub it" then "restore it again") — never converging.

  Investigated a possible mechanical fallback for the execution-ceiling sub-type (deterministically re-indent after a specific `IndentationError` shape) using `docopt.py`'s real corruption as the test case — found it's not safely buildable as a text-level heuristic: the corrupted block's own internal relative indentation stayed correct, only its depth *relative to its own `def` header* was wrong, and nothing in the indentation alone distinguishes where that block ends from where the next, correctly-indented sibling method begins (both land at the same indent level). A real fix would need to reach into `apply_symbol_blocks`'s splice context (knowing which symbol, and its real original depth) rather than pattern-matching the broken text alone — and even then risks producing plausible-but-wrong code on a bad guess, a worse outcome than today's honest `NEEDS_REVIEW`. Decision: do not build it; accept these as correctly-terminal outcomes.
- **Bugs found:** none — investigation only, no code changed.
- **Status:** complete.

### 15. Rule-based fixes round: `docopt`, `html2text`, `blinker`, `argcomplete`

- **Date:** 2026-07-23
- **Why this run:** a breakdown of the 14 open `NEEDS_REVIEW` files from earlier stress testing identified 3 concrete, scoped rule-based fixes (as opposed to a model-capability/repair-budget issue, deferred to a separate phase): docopt's Mode B sibling-import failure, html2text's dead `True`/`False` shim, and blinker/argcomplete's class-only files having nothing for Mode C to characterize. This run scopes and validates all 3, cheapest/most-certain first.

**docopt (`/private/tmp/shiftcode-stress/docopt/`):** the original diagnosis (entry 1: "Mode B's single-file isolation breaking on a sibling import") predates bugs #13/#15/#21/#22's real dependency-closure machinery and was never re-validated after they landed. Re-running showed `run_mode_b` already correctly wires `dependency_closure`/`module_rel_path` - the actual remaining bug was one level deeper: `resolve_local_imports` parsed *raw* `original_source` with stdlib `ast.parse`, which chokes on `example.py`'s own `print options` statement (valid py2, invalid py3 syntax) before it ever reaches the `from docopt import docopt` line, silently degrading to zero import edges (bug #30, fixed). Confirmed live: py2-side sandbox execution went from a raw `ImportError` to real, correct output. `example.py` still doesn't verify - a *different*, unrelated bug: `docopt.py`'s own Refactorer repair (reverting a `long`→`int` mechanical-fixer collision) corrupts the `Option.__init__` method's indentation, identically, reproduced twice in immediate succession (not just once). This directly contradicts entry 1's update, which attributed an identical-looking failure to "real LLM output non-determinism, ruled out by direct re-run" - worth re-examining; flagged as a real candidate for the model-limit investigation phase, alongside `blinker/_saferef.py`'s `__cmp__` rewrite.

**html2text (`/private/tmp/shiftcode-stress/html2text/`):** bug #14 gave the dead `True`/`False` shim a clear diagnosis but never actually removed it, so `html2text.py` was permanently stuck on this one construct. `strip_dead_true_false_shim` (bug #31, fixed) now strips it before parsing is ever attempted. Confirmed live: the file no longer hits the `SyntaxError` backstop at all, and proceeds into a real repair loop. It still doesn't fully verify - but for a completely different, newly-surfaced reason: Mode B's PASS/FAIL comparison requires stdout **and** stderr **and** exit code to match exactly; Python 3 emits a `SyntaxWarning` (`'\s+'` invalid escape sequence) on import that Python 2 doesn't, so two behaviorally-identical runs (same stdout, same exit code 0) still fail purely on stderr noise. **Flagged, not fixed** (later fixed for real — see bug #34) - this is a real, general Mode B correctness gap (likely affects other libraries' `__main__` scripts too, not just html2text), out of scope for this round.

**blinker (`/private/tmp/shiftcode-stress/blinker/`):** `blinker/base.py` is class-only (`Signal.receivers_for` and friends) - Mode C's `top_level_function_defs` structurally never found anything to characterize. New `top_level_class_defs`/`class_init`/`public_methods` (bug #32, fixed) extend Mode C to construct an instance and call a method on it. First run after this landed: 10 cases generated (proving the new discovery/prompt/driver-script path all worked end to end), but all 6 real mismatches turned out to be a *second*, more general bug this directly surfaced: `_values_equal`'s non-literal fallback compared default object/generator reprs verbatim, including a real memory address that can never match across two separate interpreter processes (bug #33, fixed). Re-run after both fixes: `blinker/base.py` reaches `VERIFIED_INFERRED`, 6/6 Mode C cases passing.

**argcomplete (`/private/tmp/shiftcode-stress/argcomplete/`):** `argcomplete/my_argparse.py` remains `UNVERIFIED` - confirmed this is the *correct* outcome, not a gap in the new capability: its one class, `IntrospectiveArgumentParser`, has exactly one method, `_parse_known_args`, an internal override of argparse's own private API, correctly excluded by the same "not underscore-prefixed" filter every other public-surface discovery in this codebase already uses. The new machinery is independently confirmed working via a second file in the same repo: `argcomplete/completers.py`'s `ChoicesCompleter` class is now discovered (its only method, `__call__`, is correctly excluded as a dunder). That same run reproduced two already-known, already-deferred issues unrelated to this work: `completers.py`'s undiagnosed `EnvironCompleter` mismatch (a top-level function, not a method - closed in entry 17) and `argcomplete/__init__.py`'s `import pipes` fix failing to survive 3 repair attempts (same model-repair-quality category as the docopt finding above, not a rules gap).

- **Bugs found:** [#30](bug-log.md), [#31](bug-log.md), [#32](bug-log.md), [#33](bug-log.md) (all found and fixed this run).
- **Status:** complete. 3/3 scoped rule-based fixes real and confirmed; 2 further real bugs found and fixed along the way (#30's closure depth, #33's repr-noise); 2 new findings flagged (Mode B stderr strictness, docopt's reproducible indentation corruption) since both are separate design/model-capability questions, not rules gaps — the Mode B one was later fixed for real (bug #34).

### 14. `toolz` (Mode R stress test)

- **Date:** 2026-07-23
- **Why this run:** [pytoolz/toolz](https://github.com/pytoolz/toolz)'s real `dicttoolz.core.merge` — first real third-party stress test of Mode R/record-replay (`docs/verification.md`), which had previously only been validated against a small hand-written example. Recorded real calls under real Python 2, replayed against toolz's real implementation and a deliberately broken one.
- **Outcome:** found and fixed a real, serious correctness bug in the recording wire format itself (JSON silently coercing non-string dict keys) before it could ever produce a false mismatch on real code. After the fix: 5/5 real recorded `merge()` calls correctly matched toolz's real implementation (`VERIFIED_RECORDED`-equivalent PASS); 4/5 correctly failed against a deliberately broken candidate, with the 5th (a single-dict merge, structurally unaffected by the specific bug introduced) correctly still passing — real per-case signal, not a blanket failure.
- **Bugs found:** [#29](bug-log.md) (found and fixed this run).
- **Status:** complete.

### 13. `argcomplete`

- **Date:** 2026-07-23
- **Why this run:** [kislyuk/argcomplete](https://github.com/kislyuk/argcomplete) @ `f6a7bf4` — third library scouted to stress differential fuzzing/confidence counts on unseen code, last of this round.
- **Outcome:** `__init__.py`'s real 43-line test suite (`test/test.py`, singular directory) was invisible to Mode A entirely until a directory-naming gap was fixed; then hit a real `import pipes` crash (removed in Python 3.13) until a new detector was added. Landed `NEEDS_REVIEW` in the end — not from a ShiftCode gap, but a real, observed Auditor misdiagnosis (told the Refactorer to restore the exact broken `pipes` import after a correct fix), an LLM-judgment limitation, same category as `docopt`'s documented non-determinism (entry 1). `completers.py` FAIL from fuzzing looked like a real, separate, undiagnosed issue at the time — root-caused later in entry 17.
- **Bugs found:** [#26](bug-log.md), [#27](bug-log.md) (both found and fixed this run).
- **Status:** complete.

### 12. `blinker`

- **Date:** 2026-07-23
- **Why this run:** [jek/blinker](https://github.com/jek/blinker) @ `c06a79a` — second library scouted to stress differential fuzzing/confidence counts on unseen code (`--characterization-fuzz-cases 30 --capture-repair-history`).
- **Outcome:** `_utilities.py` went from `VERIFIED_INFERRED` (Mode C fuzzing) to real `VERIFIED` (2/2, Mode A) once a real test-pairing gap was fixed. `_saferef.py` correctly routed to Mode A too, and correctly landed `NEEDS_REVIEW` on a genuine, hard `__cmp__`→rich-comparison rewrite the configured model couldn't complete within the repair budget — an honest degrade, not a bug; a new detector now at least surfaces the full scope of that fix upfront instead of the Auditor discovering it late.
- **Bugs found:** [#24](bug-log.md), [#25](bug-log.md) (both found and fixed this run).
- **Status:** complete.

### 11. `toolz`

- **Date:** 2026-07-23
- **Why this run:** [pytoolz/toolz](https://github.com/pytoolz/toolz) @ `498fefa` — first library scouted specifically to stress differential fuzzing and the confidence-count fields against genuinely unseen real code (`--characterization-fuzz-cases 30 --capture-repair-history`), not a re-run. Real deep nested multi-package structure (`toolz/itertoolz/`, `toolz/dicttoolz/`, `toolz/functoolz/`, each with their own `tests/` subdir).
- **Outcome:** 5 files (`dicttoolz/core.py`, `functoolz/core.py`, `itertoolz/recipes.py`, `utils.py`, `curried.py`) went from 0 verified to real `VERIFIED` via genuine Mode A runs, after three real bugs were found and fixed (two dependency-closure gaps, one missing semantic-findings detector). `itertoolz/core.py`'s one remaining failure traced to a pre-existing resource-hungry test in toolz's own suite (`range(1_000_000_000)` on Python 2 — confirmed OOM-killed directly against the real sandbox), not a ShiftCode bug.
- **Bugs found:** [#21](bug-log.md), [#22](bug-log.md), [#23](bug-log.md) (all found and fixed this run).
- **Status:** complete.
...
**Update (2026-07-23, entry 15):** the shim is now actually removed (bug #31), not just diagnosed - the file no longer hits this `SyntaxError` at all. Still `NEEDS_REVIEW`, now for an unrelated Mode B stderr-comparison false positive (flagged in entry 15, later fixed for real - see bug #34).
...
**Update (2026-07-23, entry 15):** re-validated `example.py`'s "Mode B
sibling-import" diagnosis specifically - found it was a deeper bug than
originally described (#30, fixed), and `example.py`'s py2-side sandbox run
now genuinely works. It still doesn't verify: `docopt.py`'s own repair hit
the *identical* indentation-corruption failure described above, reproduced
twice in immediate succession this time (not once) - directly at odds with
this update's earlier "ruled out by direct re-run" conclusion for what
looked like the same failure. Confirmed as a genuine model-capability
ceiling, not non-determinism, by entry 16's dedicated investigation.

## How to add an entry

Follow `docs/stress-test-methodology.md` — that's the canonical process
(find, run, diagnose, design, log both directions, confirm twice). For this
file specifically: **every entry needs both parts.**

1. **Summary table row** — one line per column, Outcome included. If it
   doesn't fit in a sentence, it belongs in the detail section, not the
   table cell. Bump the table newest-first; never renumber old entries.
2. **Entry detail section** (`### N. Title`) — date, source (repo/commit and
   why this target was chosen), full outcome, bugs found, status. This is
   where the real substance lives. A crashed/blocked run still gets both a
   row and a section — the point is an honest record, not a highlight reel.

Both parts are mandatory for every new entry, no exceptions — a table row
with no detail section (or a detail section that duplicates the table's
prose instead of the table staying short) defeats the reason this file has
two parts in the first place.
